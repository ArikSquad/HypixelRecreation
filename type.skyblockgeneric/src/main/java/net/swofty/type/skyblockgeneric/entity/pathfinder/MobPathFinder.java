package net.swofty.type.skyblockgeneric.entity.pathfinder;
...
public final class MobPathFinder {
    ...
    public enum PathType {
        BLOCKED(-1.0F), OPEN(0.0F), WALKABLE(0.0F), WALKABLE_DOOR(0.0F), TRAPDOOR(0.0F), POWDER_SNOW(-1.0F), ON_TOP_OF_POWDER_SNOW(0.0F), FENCE(-1.0F), LAVA(-1.0F), WATER(8.0F), WATER_BORDER(8.0F), RAIL(0.0F), UNPASSABLE_RAIL(-1.0F), FIRE_IN_NEIGHBOR(8.0F), FIRE(16.0F), DAMAGING_IN_NEIGHBOR(8.0F), DAMAGING(-1.0F), DOOR_OPEN(0.0F), DOOR_WOOD_CLOSED(-1.0F), DOOR_IRON_CLOSED(-1.0F), BREACH(4.0F), LEAVES(-1.0F), STICKY_HONEY(8.0F), COCOA(0.0F), DAMAGE_CAUTIOUS(0.0F), ON_TOP_OF_TRAPDOOR(0.0F), BIG_MOBS_CLOSE_TO_DANGER(4.0F);
    ...
    static class Node {
        ...
        Node(int x, int y, int z) {
            ...
            this.y = y;
            this.z = z;
            this.hash = y & 0xFF | (x & 32767) << 8 | (z & 32767) << 24 | (x < 0 ? Integer.MIN_VALUE : 0) | (z < 0 ? 32768 : 0);
        }
    ...
    public static final class Settings {
        ...
        public final Set<RegionType> allowedRegions;
        public final Map<PathType, Float> malusOverrides = new HashMap<>();

        public Settings(float width, float height, boolean canFloat, Set<RegionType> allowedRegions) {
            this.width = width;
            this.height = height;
            this.canFloat = canFloat;
            this.allowedRegions = Set.copyOf(allowedRegions);
        }
    ...
    private MobPathFinder(Instance instance, Settings settings, Point position, boolean onGround) {
        ...
        this.entityWidth = floor(settings.width + 1.0F);
        this.entityHeight = floor(settings.height + 1.0F);
        this.entityDepth = floor(settings.width + 1.0F);
    }

    public static List<Point> findPath(Instance instance, LivingEntity entity, Settings settings,
                                       Point target, float maxPathLength) {
        MobPathFinder finder = new MobPathFinder(instance, settings, entity.getPosition(), entity.isOnGround());
        return finder.find(target, maxPathLength);
    }

    private Block block(int x, int y, int z) {
        if (y < instance.getCachedDimensionType().minY() || y >= instance.getCachedDimensionType().minY() + instance.getCachedDimensionType().height()) {
            return Block.AIR;
        }
    ...
    private PathType pathTypeFromState(int x, int y, int z) {
        ...
        Material material = b.registry().material();

        RegistryTag<Block> trapdoors = Block.staticRegistry().getTag(BlockTags.TRAPDOORS);
        RegistryTag<Block> rails = Block.staticRegistry().getTag(BlockTags.RAILS);
        RegistryTag<Block> fences = Block.staticRegistry().getTag(BlockTags.FENCES);
        RegistryTag<Block> walls = Block.staticRegistry().getTag(BlockTags.WALLS);
        RegistryTag<Block> gates = Block.staticRegistry().getTag(BlockTags.FENCE_GATES);
        RegistryTag<Block> leaves = Block.staticRegistry().getTag(BlockTags.LEAVES);
        RegistryTag<Block> campfires = Block.staticRegistry().getTag(BlockTags.CAMPFIRES);
        RegistryTag<Block> doors = Block.staticRegistry().getTag(BlockTags.DOORS);

        boolean isTrapdoor = doesRegistryTagContainBlock(b, trapdoors);
        boolean isDoor = doesRegistryTagContainBlock(b, doors);
        boolean isRail = doesRegistryTagContainBlock(b, rails);
        boolean isFence = doesRegistryTagContainBlock(b, fences);
        boolean isWall = doesRegistryTagContainBlock(b, walls);
        boolean isGate = doesRegistryTagContainBlock(b, gates);
        boolean isLeaf = doesRegistryTagContainBlock(b, leaves);
        boolean isCampfire = doesRegistryTagContainBlock(b, campfires);

        if (isTrapdoor || material == Material.LILY_PAD || blockKey == BlockKeys.BIG_DRIPLEAF) return PathType.TRAPDOOR;
        if (blockKey == BlockKeys.POWDER_SNOW) return PathType.POWDER_SNOW;
        if (material == Material.CACTUS || material == Material.SWEET_BERRIES) return PathType.DAMAGING;
        if (material == Material.HONEY_BLOCK) return PathType.STICKY_HONEY;
        if (material == Material.COCOA_BEANS) return PathType.COCOA;
        if (material == Material.WITHER_ROSE || material == Material.POINTED_DRIPSTONE) return PathType.DAMAGE_CAUTIOUS;
        if (blockKey == BlockKeys.LAVA) return PathType.LAVA;
        if (blockKey == BlockKeys.FIRE || blockKey == BlockKeys.SOUL_FIRE || blockKey == BlockKeys.MAGMA_BLOCK || (isCampfire && "true".equals(b.getProperty("lit"))))
            return PathType.FIRE;
        if (isDoor) {
            boolean open = "true".equals(b.getProperty("open"));
            if (open) return PathType.DOOR_OPEN;
            return blockKey == BlockKeys.IRON_DOOR ? PathType.DOOR_IRON_CLOSED : PathType.DOOR_WOOD_CLOSED;
        }
        if (isRail) return PathType.RAIL;
        if (isLeaf) return PathType.LEAVES;
        if (isFence || isWall || (isGate && !"true".equals(b.getProperty("open")))) return PathType.FENCE;
        if (blockKey == BlockKeys.WATER) return PathType.WATER;
        if (b.isSolid()) return PathType.BLOCKED;
        return PathType.OPEN;
    }

    private boolean doesRegistryTagContainBlock(Block block, RegistryTag<Block> tag) {
        return tag != null && tag.contains(block);
    }

    private PathType pathTypeStatic(int x, int y, int z) {
        PathType type = pathTypeFromState(x, y, z);
        if (type == PathType.OPEN && y > minY()) {
    ...
    private PathType getPathTypeOfMob(int x, int y, int z) {
        ...
        for (int dx = 0; dx < entityWidth; dx++) {
            for (int dy = 0; dy < entityHeight; dy++) {
                for (int dz = 0; dz < entityDepth; dz++) {
                    ...
                    if (type == PathType.DOOR_WOOD_CLOSED && settings.canOpenDoors && settings.canPassDoors) {
                        type = PathType.WALKABLE_DOOR;
                    }
                    if (type == PathType.DOOR_OPEN && !settings.canPassDoors) {
                        type = PathType.BLOCKED;
                    }
                    if (type == PathType.RAIL && pathTypeStatic(mx, my, mz) != PathType.RAIL && pathTypeStatic(mx, my - 1, mz) != PathType.RAIL) {
        ...
        float highestMalus = settings.malus(highest);
        for (PathType type : types) {
            float malus = settings.malus(type);
        ...
        if (entityWidth > 1) {
            boolean cheaper = settings.malus(current) < highestMalus;
            boolean cap = cheaper && settings.malus(PathType.BIG_MOBS_CLOSE_TO_DANGER) < highestMalus;
            return cap ? PathType.BIG_MOBS_CLOSE_TO_DANGER : highest;
        }
        return current == PathType.OPEN && highest != PathType.OPEN && highestMalus == 0.0F ? PathType.OPEN : highest;
    }

    private PathType cachedPathType(int x, int y, int z) {
        if (!settings.allowedRegions.isEmpty()) {
            SkyBlockRegion region = SkyBlockRegion.getRegionOfPosition(new Vec(x + 0.5, y, z + 0.5));
            if (region == null || !settings.allowedRegions.contains(region.getType())) return PathType.BLOCKED;
        }
    ...
    private double getFloorLevel(int x, int y, int z) {
        if (settings.canFloat && block(x, y, z) == Block.WATER) return y + 0.5;
    ...
    private List<Point> find(Point targetPoint, float maxPathLength) {
        ...
        if (settings.canFloat && block(floor(mobX), startY, floor(mobZ)) == Block.WATER) {
        ...
        from.costMalus = settings.malus(from.type);
        ...
        Node node = best;
        while (node != null) {
            // A node's integer Y identifies its passable cell. The entity's feet
            // belong on the collision surface below it (e.g. y - 0.5 on a slab),
            // not always at that integer. Emitting the real surface height also
            // lets followers reliably finish half-block waypoints.
            points.addFirst(new Vec(node.x + 0.5, getFloorLevel(node.x, node.y, node.z), node.z + 0.5));
            node = node.cameFrom;
        }
        // Vanilla steers to the entity itself at the end instead of visibly snapping
        // toward the centre of the target's block.
        if (target.reached && !points.isEmpty()) {
            Point last = points.getLast();
            points.set(points.size() - 1, new Vec(targetPoint.x(), last.y(), targetPoint.z()));
        }
        return points;
    }


    private static final int[][] HORIZONTALS = {{0, 1}, {-1, 0}, {0, -1}, {1, 0}};
    ...
    private int getNeighbors(Node pos) {
        ...
        if (settings.malus(above) >= 0.0F && current != PathType.STICKY_HONEY) {
            jumpSize = floor(Math.max(1.0F, settings.maxUpStep));
        }
        double floorLevel = getFloorLevel(pos.x, pos.y, pos.z);

        for (int d = 0; d < 4; d++) {
            Node node = findAcceptedNode(pos.x + HORIZONTALS[d][0], pos.y, pos.z + HORIZONTALS[d][1], jumpSize, floorLevel, d, current);
        ...
        for (int d = 0; d < 4; d++) {
            ...
            if (isDiagonalValid(pos, ew, ns)) {
                Node diagonal = findAcceptedNode(pos.x + HORIZONTALS[d][0] + HORIZONTALS[cw][0], pos.y, pos.z + HORIZONTALS[d][1] + HORIZONTALS[cw][1], jumpSize, floorLevel, d, current);
                if (diagonal != null && !diagonal.closed && diagonal.type != PathType.WALKABLE_DOOR && diagonal.costMalus >= 0.0F) {
    ...
    private boolean isDiagonalValid(Node pos, Node ew, Node ns) {
        ...
        if (settings.width > 1.0F && (ew.costMalus > 0.0F || ns.costMalus > 0.0F)) return false;
        boolean canPassBetweenPosts = ns.type == PathType.FENCE && ew.type == PathType.FENCE && settings.width < 0.5;
        return (ns.y < pos.y || ns.costMalus >= 0.0F || canPassBetweenPosts) && (ew.y < pos.y || ew.costMalus >= 0.0F || canPassBetweenPosts);
    }

    private Node findAcceptedNode(int x, int y, int z, int jumpSize, double nodeHeight, int direction, PathType currentType) {
        Node best = null;
        double floorLevel = getFloorLevel(x, y, z);
        if (floorLevel - nodeHeight > Math.max(1.125, settings.maxUpStep)) return null;
        PathType type = cachedPathType(x, y, z);
        float cost = settings.malus(type);
        if (cost >= 0.0F) {
            best = getNode(x, y, z);
            best.type = type;
        ...
        if ((best == null || best.costMalus < 0.0F) && jumpSize > 0 && type != PathType.FENCE && type != PathType.UNPASSABLE_RAIL && type != PathType.TRAPDOOR && type != PathType.POWDER_SNOW) {
            best = findAcceptedNode(x, y + 1, z, jumpSize - 1, nodeHeight, direction, currentType);
        } else if (type == PathType.WATER && !settings.canFloat) {
            int yy = y - 1;
            while (yy > minY()) {
                ...
                best = getNode(x, yy, z);
                best.type = localType;
                best.costMalus = Math.max(best.costMalus, settings.malus(localType));
                yy--;
            }
        } else if (type == PathType.OPEN) {
            for (int yy = y - 1; yy >= minY(); yy--) {
                if (y - yy > settings.maxFallDistance) {
                    return blockedNode(x, yy, z);
                }
                PathType localType = cachedPathType(x, yy, z);
                float localCost = settings.malus(localType);
